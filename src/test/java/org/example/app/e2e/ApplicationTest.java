...
class ApplicationTest {
  ...
  @Test
  void End2EndTest() {
    log.debug("Db host: {}", postgresContainer.getHost());
    log.debug("Db port: {}", postgresContainer.getFirstMappedPort());
    log.debug("PostgreSQL is running at: {}", postgresContainer.getJdbcUrl());
    ...
    ResponseEntity<FileDto> uploadResponse1 =
        restTemplate.postForEntity(
            "http://localhost:" + port + "/second-memory/files/upload", mockFile1, FileDto.class);
    ...
    FileDto updateResponse =
        restTemplate.patchForObject(
            "http://localhost:" + port + "/second-memory/files/patch/1",
            updatedMockUser,
            FileDto.class);
    ...
    assertEquals(
        updatedMockUser.getName(), Objects.requireNonNull(getUserResponse.getBody()).name());
    ...
    assertEquals(
        updatedMockUser.getName(), Objects.requireNonNull(deleteResponse.getBody()).name());
